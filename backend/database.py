...
class UnifiedCursor:
    ...
    def execute(self, query: str, params: Optional[tuple] = None):
        if self._is_postgres:
            # Transform SQLite '?' to PostgreSQL '%s' if necessary
            # (only when not already containing %s)
...
def get_raw_connection() -> UnifiedConnection:
    ...
    if IS_POSTGRES:
        # Render connection strings may start with postgres:// instead of postgresql://
        url = DB_URL
...
def init_db() -> None:
    """Initializes tables and indexes with engine-specific DDL syntax."""
    with get_db() as conn:
        cursor = conn.cursor()
